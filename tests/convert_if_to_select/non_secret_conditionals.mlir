...
func.func @non_secret_condition_outside_of_secret_generic_with_secret_tensor(%inp: !secret.secret<tensor<16xi16>>, %cond: i1) -> !secret.secret<tensor<16xi16>> {
  // CHECK-NEXT: %[[RESULT:.*]] = secret.generic ins(%[[INP:.*]] : !secret.secret<tensor<[[T:.*]]>>) {
  // CHECK-NEXT:   ^[[bb0:.*]](%[[ARG2:.*]]: tensor<[[T]]>):
  // CHECK-NEXT: %[[IF:.*]] = scf.if %[[COND:.*]] -> (tensor<[[T]]>) {
  // CHECK-NEXT:     %[[ADD:.*]] = arith.addi %[[ARG2]], %[[ARG2]] : tensor<[[T]]>
  // CHECK-NEXT:     scf.yield %[[ADD]] : tensor<[[T]]>
  // CHECK-NEXT:   } else {
  // CHECK-NEXT:     scf.yield %[[ARG2]] : tensor<[[T]]>
  // CHECK-NEXT:   }
