    ...
    affine.for %i = 0 to %SIZE {
      affine.for %j = 0 to %SIZE {
        ...
        scf.if %isIndexNotSame {
          ...
          // [DO-TRANSFORM] If-Transformation needed
          scf.if %isDuplicate{
            %currentDuplicate = affine.load %duplicate[0] : memref<1xi32>
            %isFirstDuplicate = arith.cmpi eq, %currentDuplicate, %MAX : i32
            // [DO-TRANSFORM] If-Transformation needed
            scf.if %isFirstDuplicate{
              affine.store %element1, %duplicate[0] : memref<1xi32>
              // [DO-TRANSFORM] Loop-Transformation would have been needed here
              // [MLIR] Early exit from the loop
            }
          }
        }
